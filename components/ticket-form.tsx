      ...
      onSubmit={e => {
        ...
        new Promise<GitHubOAuthData | undefined>(resolve => {
          ...
          .then(async data => {
            ...
            let name: string;
            if (data.type === 'token') {
              const res = await saveGithubToken({ ticketNumber: userData.ticketNumber, token: data.token });
        ...
        >
          <div className={ticketFormStyles.generateWithGithub}>
            <span className={ticketFormStyles.githubIcon}>
              <GithubIcon color="#141414" size={24} />
            </span>
            {formState === 'loading' ? (
              <LoadingDots size={4} />
            ) : (
              username || 'Gerar com GitHub'
            )}
          </div>
          {username ? (
            <span className={ticketFormStyles.checkIcon}>
              <CheckIcon color="#141414" size={24} />
            </span>
          ) : null}
        </button>
        <p className={ticketFormStyles.description}>
          Somente seus dados públicos serão usados.
        </p>
      </div>
    </form>
  );
