...
export default function Tickets({ attendee }: Props) {
    ...
    const meta = {
        title: `${attendee.name} estará na Codecon Feature`,
        image: `/api/ticket/image?params=${btoa(JSON.stringify(params))}`
    };

    return (
        <Page theme="feature" meta={meta} noPadding hideNavMenu>
...
export const getStaticProps: GetStaticProps<any> = async ({ params }) => {
    ...
    const attendee = await Attendee.findByGithubUsernameAndEvent(user as string, 'FEATURE');
...
export const getStaticPaths: GetStaticPaths = async () => {
    const attendees = await Attendee.findAllWithGithubUsernameAndEvent('FEATURE');
