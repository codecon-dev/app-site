...
export default function TicketActions({ username, ticketNumber }: Props) {
  ...
  const tweetUrl = `https://twitter.com/intent/tweet?url=${permalink}&via=codecondev&text=${text}`;
  const linkedInUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${permalink}`;
  const downloadUrl = `/api/ticket-images/${ticketNumber}`;
  ...
  return (
    <>
      <a
        ...
        target="_blank"
      >
        <IconTwitter color="#141414" size={24} /> Tweetar
      </a>
      <a
        className={cn(
          styles.button,
        ...
        target="_blank"
      >
        <LinkedinIcon size={20} color="#141414" /> Linkedin
      </a>
      <a
