...
export default async function githubOAuth(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (githubData[0]) {
    res.end(renderSuccess({ type: 'token', token: githubData[0].token }));
    return;
  }

  await query(
    `
    INSERT INTO github (id, username, name, token)
    VALUES (?, ?, ?, ?)
    `,
    [user.id, user.login, user.name, token]
  )
